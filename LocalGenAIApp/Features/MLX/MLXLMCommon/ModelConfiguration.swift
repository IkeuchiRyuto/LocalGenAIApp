...
public struct MLXModelConfiguration: Sendable {
  ...
  public init(
    ...
    let directoryPath = documentDirectoryFileURL.appendingPathComponent(
      "mlx_model", isDirectory: true)
    do {
      try fileManager.createDirectory(
        at: directoryPath, withIntermediateDirectories: true, attributes: nil)
    } catch {
      print("失敗した")
    }
