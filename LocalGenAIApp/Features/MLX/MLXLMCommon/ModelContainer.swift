...
///
/// Wrap models used by e.g. the UI in a MLXModelContainer. Callers can access
/// the model and/or tokenizer (any values from the ``ModelContext``):
///
/// ```swift
/// let messages = [["role": "user", "content": prompt]]
/// let promptTokens = try await MLXModelContainer.perform { context in
...
/// let result = await MLXModelContainer.perform { context in
