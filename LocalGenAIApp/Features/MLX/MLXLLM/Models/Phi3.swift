...
private class Attention: Module {
  ...
  public init(_ args: Phi3Configuration) {
    ...
    self._wqkv.wrappedValue = Linear(
      dim, (heads + 2 * kvHeads) * headDim, bias: false)
    ...
    if let ropeScaling = args.ropeScaling,
      ...
      let shortFactor = ropeScaling.shortFactor,
      let longFactor = ropeScaling.longFactor
    {
      self.rope = .suScaledRotaryEmbedding(
        SuScaledRotaryEmbedding(
          ...
          originalMaxPositionEmbeddings: args
      ...
      self.rope = .rope(
        RoPE(
          dimensions: ropeDim, traditional: args.ropeTraditional,
          base: args.ropeTheta,
          scale: ropeScale))
    }
  }
  ...
  public func callAsFunction(
    ...
    queries = queries.reshaped(B, L, args.attentionHeads, -1).transposed(
      0, 2, 1, 3)
...
private class MLP: Module, UnaryLayer {
  ...
  public init(dimensions: Int, hiddenDimensions: Int) {
    self._gate_up.wrappedValue = Linear(
      dimensions, 2 * hiddenDimensions, bias: false)
    self._down.wrappedValue = Linear(
      hiddenDimensions, dimensions, bias: false)
  }
...
private class Phi3ModelInner: Module {
  ...
  public func callAsFunction(_ inputs: MLXArray, cache: [KVCache]?)
    -> MLXArray
  {
...
public struct Phi3Configuration: Codable, Sendable {
  ...
  public init(from decoder: Decoder) throws {
    ...
    hiddenSize = try container.decode(
    ...
    kvHeads = try container.decode(
      Int.self, forKey: Phi3Configuration.CodingKeys.kvHeads)
    ropeTheta =
    ...
    ropeTraditional =
      try container.decodeIfPresent(
        Bool.self, forKey: Phi3Configuration.CodingKeys.ropeTraditional)
      ?? false
